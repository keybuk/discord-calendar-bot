...
class Bot {
  ...
  async findNextEvents() {
    ...
    for (const role in roles) {
      let rsvp = roles[role];
      if (!rsvp.invited.length && !rsvp.inviteEarly) {
    ...
    if (event.description) {
      ...
      for (const line of lines) {
        if (line.toLowerCase().startsWith('image:')) {
          ...
          rsvp.invite = invite;
        } else if (line.toLowerCase().startsWith('inviteearly:')) {
          rsvp.inviteEarly = true;
        } else {
          newLines.push(line);
        }
      }
  ...
  refreshRsvp(rsvp) {
    ...
    if (rsvp.end && rsvp.end < now) {
      ...
      hide = true;
      past = true;
    } else if (rsvp.invited.length || rsvp.inviteEarly)
    ...
    const fromNow = rsvp.start.fromNow();
