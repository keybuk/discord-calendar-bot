...
class Bot {
  ...
  async syncInvites(rsvp, event) {
    ...
    for (const attendee of attendees) {
      ...
      this.log.debug(attendee, `Attendee from event, userId: ${userId}, oldResponse: ${oldResponse}, response in RSVP: ${response}`);
      if (!userId)
        continue;
      ...
      if (attendee.responseStatus !== oldResponse) {
        ...
        if (attendee.responseStatus === 'accepted') {
          ...
          this.log.debug(`Needs Action in calendar`);
        }

        this.changeRsvp(rsvp, userId, going);
      } else if (response !== oldResponse) {
