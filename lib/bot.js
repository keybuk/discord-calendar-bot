...
class Bot {
  ...
  async updateCalendarEvents() {
    await this.calendar.updateEvents(async event => {
      ...
      if (!rsvp.cancelled && !rsvp.past) {
        // Add invites to any rsvp we would announce, and then always
        // sync invites from the event to the rsvp so that it's populated.
        if (!rsvp.hide)
  ...
  async refreshAllRsvp(firstRun) {
    ...
    await this.foreachRsvp(async rsvp => {
      ...
      if (!rsvp.cancelled && !rsvp.past) {
        // Add invites to any rsvp we would announce, and if the invite set
        // has changed, then sync invites back to the event.
        if (!rsvp.hide && this.addInvites(rsvp)) {
  ...
  async syncInvites(rsvp, event) {
    ...
    if (updateAttendees) {
      try {
        this.log.debug({ attendees: attendees }, "Update event attendees");
  ...
  async rsvp(eventId, userId, going, zoom, remove) {
    ...
    if (rsvp) {
      this.changeResponse(rsvp, userId, going, zoom, remove);
