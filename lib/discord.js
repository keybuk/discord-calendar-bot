...
class Discord {
  ...
  async updateMessage(rsvp, deleteFirst) {
    ...
    if (rsvp.cancelled || rsvp.significantChange || deleteFirst) {
      if (message) {
        try {
          await message.unpin();
        } catch (err) {
      ...
      // Don't send a new message if the event is cancelled.
      if (rsvp.cancelled)
        return;
    }
    ...
    // Don't announce a future message unless it's been posted already.
    if (rsvp.future && !rsvp.posted)
      return;
    ...
    if (message) {
      ...
      this.log.debug(
        { eventId: rsvp.eventId, oldMessageId: rsvp.messageId, messageId: message.id },
        "Updated message");
    } else {
      message = await channel.send({ embeds: [content] });
      this.log.debug(
        { eventId: rsvp.eventId, messageId: message.id },
        "Announced message");
    }
    ...
    if (scheduledEvent) {
      ...
      this.log.debug(
        { eventId: rsvp.eventId, oldScheduledEventId: rsvp.scheduledEventId, scheduledEventId: scheduledEvent.id },
      ...
      if (rsvp.start > now) {
        ...
        this.log.debug(
          { eventId: rsvp.eventId, scheduledEventId: scheduledEvent.id },
  ...
  async syncInterested(rsvp) {
    ...
    for (const userId of rsvp.interested) {
      if (!subscribers.find(eU => eU.user.id === userId)) {
        const user = await this.client.users.fetch(userId);
