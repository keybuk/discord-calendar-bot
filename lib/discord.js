...
class Discord {
  ...
  constructor(log, config, calendar) {
    ...
    this.client.on('message', message => {
      ...
      if (command === 'delete') {
        this.deleteCommand(command, args, message)
          .catch(err => this.log.error(err, "Error caught during message"));
      }
    });
  ...
  getChannel(rsvp) {
    var channelName = this.bot.state.channels[rsvp.invite] || this.config.channel;
  ...
  async editCommand(command, args, message) {
    ...
    if (action.toLowerCase() === 'color') {
      ...
      if (color) {
        ...
        return message.reply("Event updated");
      } else {
        return message.reply("Unknown role");
      }
    } else {
      return message.reply("Specify one of: `color`");
    }
  }
  ...
  async deleteCommand(command, args, message) {
    const eventId = args.shift();

    var rsvp = await this.bot.getRsvp(eventId);
    if (!rsvp) {
      return message.reply("Unknown event");
    }
