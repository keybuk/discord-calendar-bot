...
class Discord {
  ...
  constructor(log, config, calendar) {
    ...
    this.client.on('message', message => {
      ...
      if (command === 'refresh') {
        this.refreshCommand(command, args, message)
          .catch(err => this.log.error(err, "Error caught during message"));
      }
    });
  ...
  async reactionAdded(reaction, user) {
    ...
    if (reaction.emoji.name === this.yesEmoji) {
      ...
      await this.rsvp(eventId, user.id, undefined);
    } else {
  ...
  async refreshCommand(command, args, message) {
    const eventId = args.shift();

    if (eventId) {
      const event = await storage.getItem('event/' + eventId);
      if (!event)
        return message.reply("I don't know that event.");

      await this.announceEvent(event);
      return message.reply("Okay, refreshed that event.");
    }


    const events = await storage.valuesWithKeyMatch(/^event\//);
    for (const event of events) {
      await this.announceEvent(event);
    }

    return message.reply(`Okay, refreshed ${events.length} events.`);
  }
